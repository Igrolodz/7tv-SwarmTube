...
async function getEmotes(){
    emoteSet = await new Promise((resolve) => {
        chrome.runtime.sendMessage({ type: "GET_7TV_EMOTES" }, (response) => {
            resolve(response.emotes);
        });
    });
    excludedSet = await chrome.storage.local.get("excludedEmotes");
}
...
async function setList(){
    ...
    excludedSet.excludedEmotes.forEach(element => {
        ...
        if(!emoteData) {
            ...
            chrome.storage.local.set({ excludedEmotes: excludedSet.excludedEmotes });
            return;
        }
...
async function removeEmote(emoteName){
    ...
    await chrome.storage.local.set({ excludedEmotes: excludedSet.excludedEmotes });
}
